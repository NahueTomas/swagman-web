...
export const escapeUrl = (url: string): string => {
  if (urlCache.has(`escape_${url}`)) {
    return urlCache.get(`escape_${url}`)!;
  }
...
export const unescapeUrl = (url: string): string => {
  if (urlCache.has(`unescape_${url}`)) {
    return urlCache.get(`unescape_${url}`)!;
  }
...
const getStatusCodesNames = () => {
  if (statusCodesNames) return statusCodesNames;
